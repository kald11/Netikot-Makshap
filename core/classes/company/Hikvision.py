...
class Hikvision(Company):
    ...
    def __init__(self, site):
        ...
        self._prefix_isapi = f"{self.site.prot}://{self.site.ip}:{self.site.nvr.port}/ISAPI/System"
        self._prefix_psia = f"http://{self.site.ip}:{self.site.nvr.port}/PSIA/System"
    ...
    def try_login(self):
        if self.flags["is_nvr_ping"]:
            try:
                res = requests.get(f"{self._prefix_isapi}/status", auth=self.site.credentials, timeout=self.timeout)
                if res.ok:
                    self.flags["login_ok"] = True

                else:
                    res = requests.get(f"{self._prefix_psia}/status", auth=self.site.credentials, timeout=self.timeout)

                    if res.status_code == 401:  # old models don't support Digest Auth
                        new_credentials = (self.username, self.password)
                        res = requests.get(f"{self._prefix_psia}/status", auth=new_credentials, timeout=self.timeout)
        ...
        try:
            ...
            if self.flags["login_ok"]:
                if self.device_info in [('TS-5012-F', 4), ("DS-TP50-12DT", 4)]:
                    res = self.session.get(f"{self._prefix_psia}/time", auth=self.site.credentials,
                                           timeout=self.timeout)
                    self.times["current_camera_time"] = self._extract_date(res)
                else:
                    res = self.session.get(f"{self._prefix_isapi}/time", auth=self.site.credentials,
                                           timeout=self.timeout)
                    self.times["current_camera_time"] = self._extract_date(res)
    ...
    def _get_device_info(self):
        if self.flags["login_ok"]:
            try:
                r = self.session.get(f"{self._prefix_isapi}/deviceInfo", auth=self.site.credentials,
                                     timeout=self.timeout)
                if r.ok:
                    r = ordered_dict_to_dict(xmltodict.parse(r.text))["DeviceInfo"]
                    self.device_info = (r["model"], int(r["firmwareVersion"][1:].split('.')[0]))
                else:
                    r = self.session.get(f"{self._prefix_psia}/deviceInfo", auth=self.site.credentials,
                                         timeout=self.timeout)
                    if r.ok:
                        r = ordered_dict_to_dict(xmltodict.parse(r.text))["DeviceInfo"]
                        self.device_info = (r["model"], int(r["firmwareVersion"][1:].split('.')[0]))
